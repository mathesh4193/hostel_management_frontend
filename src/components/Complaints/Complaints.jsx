...
const StudentComplaint = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { name: student.name, rollno: student.rollNo, roomNo: student.roomNo, ...formData };
  ...
  return (
    <Container className="py-4">
      <Card className="shadow-sm mb-4">
        <Card.Header className="bg-primary text-white"><h4>Submit Complaint</h4></Card.Header>
        <Card.Body>
          {submitStatus.show && <Alert variant={submitStatus.variant}>{submitStatus.message}</Alert>}
          <Form noValidate validated={validated} onSubmit={handleSubmit}>
            <Row className="mb-3">
              <Col md={4}><Form.Group><Form.Label>Name</Form.Label><Form.Control type="text" value={student.name || ''} readOnly /></Form.Group></Col>
              <Col md={4}><Form.Group><Form.Label>Roll Number</Form.Label><Form.Control type="text" value={student.rollNo || ''} readOnly /></Form.Group></Col>
              <Col md={4}><Form.Group><Form.Label>Room Number</Form.Label><Form.Control type="text" value={student.roomNo || ''} readOnly /></Form.Group></Col>
            </Row>
            <Row className="mb-3">
              <Col md={6}>
                <Form.Group>
              ...
              <Col md={6}>
                <Form.Group>
                  ...
                  <Form.Control required type="text" name="subject" value={formData.subject} onChange={handleChange} placeholder="Brief subject" />
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control required as="textarea" rows={4} name="description" value={formData.description} onChange={handleChange} placeholder="Detailed description" />
              <Form.Control.Feedback type="invalid">Enter a description</Form.Control.Feedback>
            </Form.Group>
            <div className="d-grid"><Button type="submit" variant="primary">{loadingSubmit ? 'Submitting...' : 'Submit Complaint'}</Button></div>
          </Form>
        </Card.Body>
      </Card>

      <Card>
        <Card.Header className="bg-secondary text-white"><h5>My Complaint History</h5></Card.Header>
        <Card.Body>
          {loadingComplaints ? <Spinner animation="border" /> :
            complaints.length === 0 ? <p>No complaints submitted yet.</p> :
              <Table striped bordered hover responsive>
                <thead><tr><th>Date</th><th>Category</th><th>Subject</th><th>Description</th><th>Status</th></tr></thead>
                <tbody>{complaints.map(c => <tr key={c._id}><td>{new Date(c.date || c.createdAt).toLocaleDateString()}</td><td>{c.category}</td><td>{c.subject}</td><td>{c.description}</td><td>{getStatusBadge(c.status)}</td></tr>)}</tbody>
              </Table>}
        </Card.Body>
      </Card>
    </Container>
